...
# Make the mock execute return a proper dict for status
...
"""
    ...
    def test_lifecycle_controller_restart(mock_lm, dispatcher):
        ...
        mock_lm.return_value.execute.assert_any_call("stop", "pkg1", False, follow=False, tails="all")
        mock_lm.return_value.execute.assert_any_call("start", "pkg1", False, follow=False, tails="all")
""",
"""
    @patch("steglib.daemon_api.LifecycleManager")
    def test_lifecycle_controller_restart(mock_lm, dispatcher):
